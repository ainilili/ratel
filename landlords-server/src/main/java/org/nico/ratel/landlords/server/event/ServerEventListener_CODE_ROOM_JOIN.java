...
public class ServerEventListener_CODE_ROOM_JOIN implements ServerEventListener{
	...
	public void call(ClientSide clientSide, String data) {
		...
		if(room == null) {
			ChannelUtils.pushToClient(clientSide.getChannel(), ClientEventCode.CODE_ROOM_JOIN_FAIL_BY_INEXIST, data);
		}else {
			...
			if(room.getClientSideList().size() == 3) {
				...
				if(roomClientMap.size() == 3) {
					...
					ChannelUtils.pushToClient(clientSide.getChannel(), ClientEventCode.CODE_ROOM_JOIN_SUCCESS, result);

					for(ClientSide client: roomClientMap.values()) {
						if(client.getId() != clientSide.getId()) {
							ChannelUtils.pushToClient(client.getChannel(), ClientEventCode.CODE_ROOM_JOIN_TRIGGER, result);
						}
					}
				}
			}
